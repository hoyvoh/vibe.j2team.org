  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-bg-deep/80 px-4">
    <div class="w-full max-w-sm border p-5" :class="surfaceClass">
      ...
      <p class="mt-2 text-sm" :class="textMutedClass">
        {{
          appliedMode === 'story' || appliedMode === 'gravity'
            ? `Bạn vừa hoàn thành ${appliedMode === 'gravity' ? 'Gravity' : 'Story'} ${storyTotalLevels} level trong ${timeLabel}. Nhập tên để lưu kỷ lục chính.`
            : 'Nhập tên để lưu bảng xếp hạng theo độ khó/mode hiện tại.'
        }}
      </p>
      <input
        :value="playerName"
        ...
        class="mt-4 w-full border bg-transparent px-3 py-2 text-sm outline-none focus:border-accent-coral"
