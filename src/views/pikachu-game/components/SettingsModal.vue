...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-bg-deep/80 px-4">
    <div class="w-full max-w-sm border p-5" :class="surfaceClass">
      ...
      <button
        type="button"
        class="mt-2 w-full border px-3 py-2 text-sm transition hover:border-accent-sky"
        :class="panelInnerClass"
      ...
      <div class="mt-2 border p-3" :class="panelInnerClass">
        <p class="text-xs font-display tracking-widest" :class="textMutedClass">
          ÂM LƯỢNG NHẠC NỀN: {{ props.bgmVolume }}%
        </p>
        <input
          ...
          max="100"
          step="1"
          class="mt-2 w-full accent-accent-coral"
          @input="emit('update:bgmVolume', Number(($event.target as HTMLInputElement).value))"
        />
      </div>
      <button
        type="button"
        class="mt-2 w-full border px-3 py-2 text-sm transition hover:border-accent-amber"
        :class="panelInnerClass"
      ...
      <div class="mt-2 border p-3" :class="panelInnerClass">
        <p class="text-xs font-display tracking-widest" :class="textMutedClass">
          ÂM LƯỢNG HIỆU ỨNG: {{ props.sfxVolume }}%
        </p>
        <input
