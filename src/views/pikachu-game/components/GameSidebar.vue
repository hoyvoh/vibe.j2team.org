...
<template>
  <aside class="animate-fade-up animate-delay-1 border p-3" :class="surfaceClass">
    ...
    <div class="grid gap-1.5">
      <button
        type="button"
        class="border px-2.5 py-1.5 text-xs transition hover:border-accent-coral"
        :class="panelInnerClass"
        @click="emit('open-new-game')"
      >
        Bắt đầu ván mới
      </button>
      <button
        type="button"
        class="border px-2.5 py-1.5 text-xs transition hover:border-accent-amber"
        :class="panelInnerClass"
      ...
      <button
        type="button"
        class="border px-2.5 py-1.5 text-xs transition hover:border-accent-sky"
        :class="panelInnerClass"
    ...
    <div class="mt-2.5 border p-2" :class="panelInnerClass">
      ...
      <input
        v-model.number="bgmModel"
        type="range"
        min="0"
        max="100"
        step="1"
        class="mt-1 w-full accent-accent-coral"
      />
      <p class="mt-2 text-[10px] font-display tracking-widest">SFX {{ sfxModel }}%</p>
      <input
        v-model.number="sfxModel"
        type="range"
        min="0"
        max="100"
        step="1"
