...
const {
  ...
  isGuessing,
  isError,
} = useDailyQuiz<TechdleItem, TechdleItem>('techdle')
    ...
    >
      ...
      <div v-if="status[currentQ] === 'playing'" class="flex flex-col sm:flex-row gap-4 mb-4">
        ...
        <button
          @click="handleGuess"
          :disabled="isGuessing"
          class="px-8 py-3 bg-accent-sky text-bg-deep font-display font-bold hover:bg-white hover:text-accent-sky transition-colors duration-300 disabled:opacity-50 flex items-center justify-center min-w-[120px]"
        >
          <span
            v-if="isGuessing"
      ...
      <div class="overflow-x-auto pb-4 custom-scrollbar">
        <div class="inline-flex flex-col gap-3 min-w-[800px] w-full">
          <!-- Header -->
          <div
            class="grid grid-cols-6 gap-3 text-center text-xs font-display text-text-dim uppercase tracking-widest pb-3 border-b border-border-default"
          >
            <div>Tên</div>
            <div>Năm ra đời</div>
            <div>Màu Logo</div>
          ...
          <TransitionGroup name="list" tag="div" class="flex flex-col gap-3">
            ...
            >
              ...
              <div
                class="p-3 border flex items-center justify-center font-bold font-display text-lg transition-colors"
                :class="getMatchColor(guess._comparison?.year)"
              >
              ...
              <div
                class="p-3 border flex items-center justify-center transition-colors"
                :class="getMatchColor(guess._comparison?.logoColor)"
              >
                {{ guess._comparison?.logoColor === 'correct' ? guess.logoColor : '???' }}
              </div>
              <div
                class="col-span-2 p-3 border flex items-center justify-center italic text-xs transition-colors"
                :class="getMatchColor(guess._comparison?.nickname)"
              >
                {{ guess._comparison?.nickname === 'correct' ? '"' + guess.nickname + '"' : '???' }}
              </div>
              <div
                class="p-3 border flex items-center justify-center font-bold font-display text-lg transition-colors"
                :class="getMatchColor(guess._comparison?.hateRatio)"
              >
