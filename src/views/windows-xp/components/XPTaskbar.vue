...
<template>
  <div
    class="absolute bottom-0 left-0 right-0 h-[30px] bg-gradient-to-b from-[#245edb] via-[#3f8cf3] to-[#245edb] flex items-center z-50 shadow-[0_-1px_1px_rgba(255,255,255,0.4)] overflow-hidden"
  >
    <!-- Start Button -->
    <button
      ...
      :class="{
        'brightness-90 bg-gradient-to-t shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)]': startMenuOpen,
      }"
    ...
    >
      ...
      <span
        class="text-xl tracking-wide -ml-1 mt-0.5"
    ...
    <!-- System Tray Area -->
    <div
      class="h-full bg-gradient-to-b from-[#0e9ae1] to-[#1281cb] border-l border-[#136eb1] shadow-[inset_1px_0_0_rgba(255,255,255,0.3)] flex items-center justify-end px-3 gap-3 ml-auto text-white flex-shrink-0"
    >
      <div class="flex gap-1">
        <div
          class="w-4 h-4 rounded-full bg-white/20 flex items-center justify-center text-[10px]"
          title="Volume"
        >
          🔊
        </div>
      ...
      <span class="text-xs cursor-default drop-shadow-md" style="font-family: Tahoma, sans-serif">{{
        currentTime
      }}</span>
    </div>
  </div>
