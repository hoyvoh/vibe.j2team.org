...
const handleTurnOff = () => {
  if (confirm('Are you sure you want to shut down?')) {
    document.documentElement.innerHTML =
      '<div style="background:black; color:white; width: 100vw; height: 100vh; display: flex; align-items:center; justify-content:center; font-family: sans-serif">It is now safe to turn off your computer.</div>'
  }
}
</script>
...
<template>
  <div
    class="xp-desktop h-screen w-full relative overflow-hidden select-none"
    @click="closeAllMenus"
    @contextmenu="onDesktopRightClick"
  >
    <!-- Back to Home Button -->
    <RouterLink
      to="/"
      class="absolute top-4 right-4 z-50 flex items-center gap-2 px-3 py-1.5 bg-gradient-to-b from-[#3c81f3] to-[#2c6bf0] border border-white/50 text-white text-sm font-bold shadow-md rounded hover:brightness-110 active:brightness-90 transition-all font-sans cursor-pointer"
      style="
    ...
    <!-- Desktop Background -->
    <div
      class="absolute inset-0 bg-[#0055e5] bg-cover bg-center z-0"
      style="
      ...
      <div v-for="win in windows" :key="win.id" class="pointer-events-auto">
        <Transition name="window-open">
          ...
          >
            ...
            <div
              v-if="win.id === 'notepad'"
              class="h-full w-full bg-white text-black p-2 font-mono text-sm leading-relaxed overflow-y-auto"
            >
              Welcome to the Windows XP vibe!
              <br /><br />
              Bạn đã mở thành công Notepad. Dưới đây là một số tính năng bạn có thể thử:
              <br />- Click đúp thanh tiêu đề để phóng to / thu nhỏ. <br />- Kéo thả cửa sổ quanh
              màn hình. <br />- Nhấn nút Start bên dưới góc trái. <br />- Mở thêm vài ứng dụng khác
              từ Desktop để xem hiệu ứng z-index. <br /><br />
              Chúc vui !!!
            </div>

            <div v-else-if="win.id === 'internet-explorer'" class="h-full w-full flex flex-col">
              <div
                class="h-8 bg-[#ece9d8] border-b border-gray-300 flex items-center px-2 gap-2 shrink-0"
              >
                <button
                  class="px-2 py-0.5 border border-transparent hover:border-gray-400 hover:shadow-sm rounded bg-white/50 text-xs flex items-center gap-1 disabled:opacity-50"
                >
                  ⬅ Back
                </button>
                <button
                  class="px-2 py-0.5 border border-transparent hover:border-gray-400 hover:shadow-sm rounded bg-white/50 text-xs flex items-center gap-1"
                >
                  ➡ Forward
                </button>
                <button
              ...
              <div
                class="h-8 bg-[#ece9d8] border-b border-gray-300 flex items-center px-2 gap-2 shrink-0 text-sm"
              >
                <span class="text-gray-600">Address</span>
                <div
                  class="flex-1 bg-white border border-gray-400 shadow-inner px-2 py-0.5 text-black h-6 flex items-center truncate"
                >
                  http://vibe.j2team.org
                </div>
                <button
                  class="px-2 py-0.5 border border-gray-400 rounded bg-gray-100 flex items-center gap-1 hover:bg-gray-200"
                >
                  Go
                </button>
              </div>
    ...
    <Transition name="fade-slide">
      <XPStartMenu v-if="startMenuOpen" @log-off="handleLogOff" @turn-off="handleTurnOff" />
    </Transition>
    ...
    <!-- Taskbar -->
    <XPTaskbar :start-menu-open="startMenuOpen" @toggle-start-menu="toggleStartMenu">
        ...
        <button
          ...
          :class="
            activeWindowId === win.id
              ? 'bg-gradient-to-b from-[#1a51c4] to-[#103482] border border-[#164ac1] shadow-[inset_0_1px_3px_rgba(0,0,0,0.6)]'
              : 'bg-gradient-to-b from-[#3c81f3] to-[#2c6bf0] border border-[#164ac1] shadow-[inset_0_1px_1px_rgba(255,255,255,0.3)] hover:from-[#5c98f5] hover:to-[#3e82f2]'
          "
          @click="focusWindow(win.id)"
        >
          <span
            class="text-xs truncate font-bold drop-shadow"
            style="font-family: Tahoma, sans-serif"
            >{{ win.icon }} {{ win.title }}</span
          >
        </button>
      </template>
    </XPTaskbar>
